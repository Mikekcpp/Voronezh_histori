...
class QuizWindow(QWidget):
        ...
        if self.currentQuestion < len(self.questions):
            ...
            if answer.lower() == question[2].lower():
                ...
                QMessageBox.information(
                    self, "Правильно!", "Вы правильно ответили на вопрос!"
                )
            else:
                QMessageBox.information(
                    self, "Неправильно!", "Вы неправильно ответили на вопрос!"
                )
    ...
    def updateScore(self):
        if self.userId is not None:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE users SET score = ? WHERE id = ?", (self.score, self.userId)
            )
            conn.commit()
            conn.close()
        else:
